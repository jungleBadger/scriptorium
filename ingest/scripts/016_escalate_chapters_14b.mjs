// ingest/scripts/016_escalate_chapters_14b.mjs
// Phase 2 chapter explanation pipeline.
// Escalates errored / low-confidence chapters from a prior Ollama run (Phase 1)
// to a stronger Ollama model (default: qwen3:14b). Full quality retry chain.
//
// Requires --source-model to identify which prior model's results to escalate from.
//
// Usage:
//   node ingest/scripts/016_escalate_chapters_14b.mjs --source-model qwen3:8b
//   node ingest/scripts/016_escalate_chapters_14b.mjs --source-model qwen3:8b --min-band medium
//   node ingest/scripts/016_escalate_chapters_14b.mjs --source-model qwen3:8b --translation PT1911 --limit 20
//   node ingest/scripts/016_escalate_chapters_14b.mjs --source-model qwen3:8b --book JOS --force
...
const DEFAULT_MODEL = process.env.CHAPTER_MODEL || process.env.OLLAMA_MODEL || "qwen3:14b";
...
const CHAPTER_OUTPUT_SCHEMA = {
    type: "object",
    properties: { chapter_explanation: { type: "string" } },
...
// Finds chapters where a prior model run produced status='error' or a low/medium
// confidence band, joins back to verses for canonical book ordering.
            ...
            .map((e) => {
                const base = {
                    ...
                    aliases: Array.isArray(e?.aliases)
                        ? e.aliases.map((a) => (typeof a === "string" ? a.trim() : "")).filter(Boolean).slice(0, MAX_ALIASES_PER_ENTITY)
                        : [],
                ...
                if (rich) base.description_rich = rich;
                else if (plain) base.description = plain;
                const modernName = e?.modern?.name;
                if (typeof modernName === "string" && modernName.trim()) base.modern = { name: modernName.trim() };
                return base;
            })
            .sort((a, b) => {
                if ((b.verse_hits ?? 0) !== (a.verse_hits ?? 0)) return (b.verse_hits ?? 0) - (a.verse_hits ?? 0);
...
function computeChapterComplexity(payload) {
    ...
    const listHeavyVerses = verses.reduce((acc, v) => {
        const commaCount = (String(v?.text || "").match(/,/g) || []).length;
    ...
    return {
        score, verse_count: verseCount, total_chars: totalChars,
        avg_verse_chars: Math.round(avgVerseChars), entity_count: entityCount,
        entity_density: Number(entityDensity.toFixed(3)),
        list_heavy_ratio: Number(listHeavyRatio.toFixed(3)), list_heavy_semantic: listHeavySemantic,
    };
}
...
async function fetchOllamaModels(host) {
    let res;
    try { res = await fetch(`${host}/api/tags`); }
    catch { throw new Error(`Cannot reach Ollama at ${host}. Start it with: ollama serve`); }
    if (!res.ok) throw new Error(`Ollama health check failed with ${res.status}`);
...
function assertModelAvailable(models, model) {
    ...
    if (!found) throw new Error(`Model "${model}" not found in Ollama. Available: ${models.join(", ") || "(none)"}`);
}
...
async function callOllama({ host, model, userPrompt, temperature, topP, numPredict }) {
    ...
    const bodySchema = {
        ...
        stream: false,
        options: { temperature, top_p: topP, num_predict: numPredict, think: false },
    };

    const res1 = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(bodySchema) });
    if (!res1.ok) throw new Error(`Ollama returned ${res1.status}: ${await res1.text()}`);
    const data1 = await res1.json();
    let text = extractOllamaText(data1).trim();
    if (text) return text;

    const res2 = await fetch(url, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...bodySchema, format: "json" }) });
    if (!res2.ok) throw new Error(`Ollama fallback returned ${res2.status}: ${await res2.text()}`);
    const data2 = await res2.json();
    text = extractOllamaText(data2).trim();
    if (text) return text;

    throw new Error(`Ollama returned empty content twice. Schema: ${summarizeKeys(data1)}. JSON: ${summarizeKeys(data2)}.`);
}
...
function countSentences(text) {
    return stripVerseRefsForCounting(text).split(/[.!?]+/).map((p) => p.trim()).filter(Boolean).length;
}
...
function countVerseRefsInBlock(blockText) {
    const matches = String(blockText || "").match(/\bv\.\s*\d[\d\-–]*/gi);
    return matches ? matches.length : 0;
}
...
function hasBookChapterVerseMention(text) {
    return /\b(?:[1-3]\s+)?[A-Za-z]{2,}(?:\s+[A-Za-z]{2,})*\s+\d{1,3}:\d{1,3}\b/.test(String(text || ""));
}
...
function evaluateVerseRefCount(explanation) {
    ...
    return {
        ok: blockCount === 1 && finalBlockAtEnd && refCount === 2 && !inlineOutsideFinal && !bookChapterVerseHit,
        block_count: blockCount, final_block_at_end: finalBlockAtEnd,
        verse_ref_count: refCount, inline_verse_outside_final_block: inlineOutsideFinal,
        book_chapter_verse_hit: bookChapterVerseHit, required_blocks: 1, required_refs: 2,
    };
}
...
function evaluateConfidence({ explanation, wordCount, wordPolicy, sentenceAssessment, verseRefAssessment, groundingAssessment, metaTalkAssessment, retries = {}, verseRefNormalized = false }) {
    ...
    else if (Number.isFinite(groundingAssessment?.required_hits) && groundingAssessment.required_hits > 0 && Number.isFinite(groundingAssessment?.token_hits) && groundingAssessment.token_hits > groundingAssessment.required_hits) {
        score += 3; reasons.push("grounding_token_hits_above_minimum");
    }
    ...
    return { score: finalScore, band, reasons };
}
...
function extractVerseGroundingTokens(payload, maxTokens = 16) {
    ...
    const pushToken = (raw) => {
        ...
        if (/[-']/g.test(token)) {
            for (const part of token.split(/[-']/g)) {
                ...
                if (partNorm.length >= 3 && !TOKEN_STOPWORDS.has(partNorm)) freq.set(partNorm, (freq.get(partNorm) || 0) + 1);
            }
        }
    };
    const verses = Array.isArray(payload?.verses) ? payload.verses : [];
    for (const verse of verses) {
        ...
        for (const cand of (text.match(/\b[A-Z][A-Za-z'-]{2,}(?:\s+[A-Z][A-Za-z'-]{2,}){0,2}\b/g) || [])) pushToken(cand);
...
function evaluateGrounding(explanation, payload, { listHeavy = false } = {}) {
    ...
    const hasListHeavyLanguage = listHeavy
        ? LIST_HEAVY_KEYWORDS.some((kw) => normalizedText.includes(kw)) || GENEALOGY_KEYWORDS.map(normalizeForGrounding).some((kw) => normalizedText.includes(kw))
        : true;
    ...
    return {
        ...
        token_hits: matchedTokens.length, required_hits: requiredHits,
        entity_density: Number(entityDensity.toFixed(3)), relaxed_for_list_or_dense: relaxedForListOrDense,
        list_heavy_keyword_hit: hasListHeavyLanguage, disallowed_phrase_hit: hasDisallowedPhrase, verse_ref_hit: hasVerseRef,
    };
}
...
async function main() {
    ...
    if (!sourceModel) throw new Error("--source-model <model> is required (the Phase 1 model whose results to escalate).");
    ...
    if (retryModel !== model) assertModelAvailable(availableModels, retryModel);
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        try {
            ...
            if (complexityInfo.score >= 5) numPredictUsed = Math.max(numPredictUsed, 1000);
            ...
            console.log(`  Model: ${selectedModel} (complexity: ${complexityInfo.score}, tier: ${payloadTier}, num_predict: ${numPredictUsed}, prompt: ${promptContext.promptVersion}, words: ${wordPolicy.target_words}/${wordPolicy.min_words}-${wordPolicy.max_words})`);
            ...
            const callAndSetParsedWithJsonGuard = async ({ modelName, userPrompt, temp, numPredictValue = numPredictUsed }) => {
                const previousParsed = parsed ? { chapter_explanation: parsed.chapter_explanation, output_json: parsed.output_json && typeof parsed.output_json === "object" ? { ...parsed.output_json } : { chapter_explanation: parsed.chapter_explanation } } : null;
                const fallbackToPrevious = () => { if (!previousParsed) return false; jsonFallbackToPrevious = true; setParsed(previousParsed); return true; };
                try {
                    rawResponse = await callOllama({ host: OLLAMA_HOST, model: modelName, userPrompt, temperature: temp, topP, numPredict: numPredictValue });
                    try { setParsed(parseChapterOutput(rawResponse)); temperatureUsed = temp; }
                        ...
                        rawResponse = await callOllama({ host: OLLAMA_HOST, model: modelName, userPrompt: repairPrompt, temperature: 0, topP, numPredict: numPredictValue });
                        try { setParsed(parseChapterOutput(rawResponse)); temperatureUsed = 0; }
                        catch {
                            if (modelName !== selectedModel) {
                                rawResponse = await callOllama({ host: OLLAMA_HOST, model: selectedModel, userPrompt: repairPrompt, temperature: 0, topP, numPredict: numPredictValue });
                                try { setParsed(parseChapterOutput(rawResponse)); temperatureUsed = 0; }
                                catch {
            ...
            rawResponse = await callOllama({ host: OLLAMA_HOST, model: selectedModel, userPrompt: prompt, temperature, topP, numPredict: numPredictUsed });
            ...
            if (isLikelyTruncated(parsed.chapter_explanation)) {
                ...
                await callAndSetParsedWithJsonGuard({ modelName: selectedModel, userPrompt: `${prompt}\n\nIMPORTANT: Ensure the explanation is complete and ends with a full sentence. Output only valid JSON.`, temp: 0, numPredictValue: numPredictUsed });
            }
            ...
            if (wordCount < wordPolicy.min_words) {
                tooshortRetry = true; numPredictUsed = Math.max(numPredictUsed, 1100);
                await callAndSetParsedWithJsonGuard({ modelName: selectedModel, userPrompt: `${prompt}\n\n${buildVerbosityRetryNote(wordPolicy)}`, temp: verbosityRetryTemperature, numPredictValue: numPredictUsed });
                wordCount = countWords(parsed.chapter_explanation); lastWordCount = wordCount;
                if (wordCount < wordPolicy.min_words) {
                    tooshortRetry2 = true; numPredictUsed = Math.max(numPredictUsed, 1300);
                    await callAndSetParsedWithJsonGuard({ modelName: selectedModel, userPrompt: `${prompt}\n\n${buildVerbosityRetryNote(wordPolicy, { strict: true })}`, temp: verbosityRetryTemperature, numPredictValue: numPredictUsed });
                    wordCount = countWords(parsed.chapter_explanation); lastWordCount = wordCount;
                }
            }
            ...
            { const preNorm = evaluateVerseRefCount(parsed.chapter_explanation); if (!preNorm.ok) { const normalised = consolidateVerseRefs(parsed.chapter_explanation); if (normalised !== parsed.chapter_explanation) { verseRefNormalized = true; parsed.chapter_explanation = normalised; parsed.output_json.chapter_explanation = normalised; } } }

            wordCount = countWords(parsed.chapter_explanation); lastWordCount = wordCount;
            if (wordCount < wordPolicy.min_words) {
                postFormatRecoveryRetry = true; numPredictUsed = Math.max(numPredictUsed, 1300);
                await callAndSetParsedWithJsonGuard({ modelName: selectedModel, userPrompt: `${prompt}\n\n${buildVerbosityRetryNote(wordPolicy, { strict: true })}`, temp: Math.max(verbosityRetryTemperature, 0.45), numPredictValue: numPredictUsed });
                { const postRecovNorm = evaluateVerseRefCount(parsed.chapter_explanation); if (!postRecovNorm.ok) { const normalised = consolidateVerseRefs(parsed.chapter_explanation); if (normalised !== parsed.chapter_explanation) { verseRefNormalized = true; parsed.chapter_explanation = normalised; parsed.output_json.chapter_explanation = normalised; } } }
            }
            ...
            if (finalWordCount < wordPolicy.min_words) {
                finalLengthRescueRetry = true; numPredictUsed = Math.max(numPredictUsed, 1400);
                await callAndSetParsedWithJsonGuard({ modelName: retryModel, userPrompt: `${prompt}\n\nIMPORTANT: Rewrite from scratch. Keep AT LEAST ${wordPolicy.min_words} words (target ${wordPolicy.target_words}). Use ${listHeavy ? "EXACTLY 3 sentences" : "BETWEEN 4 and 6 sentences"}. Include exactly 2 verse references total in ONE parenthesized block and no other "(v." anywhere. The verse-reference block must be at the end. Output valid JSON only.`, temp: 0, numPredictValue: numPredictUsed });
                { const postRescueNorm = evaluateVerseRefCount(parsed.chapter_explanation); if (!postRescueNorm.ok) { const normalised = consolidateVerseRefs(parsed.chapter_explanation); if (normalised !== parsed.chapter_explanation) { verseRefNormalized = true; parsed.chapter_explanation = normalised; parsed.output_json.chapter_explanation = normalised; } } }
            ...
            parsed.output_json._meta = {
                ...
                temperature_used: temperatureUsed, top_p: topP, num_predict_used: numPredictUsed,
    ...
    await client.end();
    console.log("");
    console.log("Chapter escalation enrichment finished.");
